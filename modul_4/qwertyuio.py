...
BOT_TOKEN = '6188011362:AAHpdXOLFTTdJbQKnUB6oVC_SbL5VAbYLXQ'
...
async def process_admin_callback(callback_query: types.CallbackQuery):
    cur = conn.cursor()
    cur.execute("SELECT is_admin FROM users WHERE user_id = %s", (str(callback_query.from_user.id),))
    result = cur.fetchone()
    if result and result[0]:
        ...
        await bot.send_message(callback_query.from_user.id,
                               'Siz admin emassiz. Admin panelga kirish uchun ruxsatnoma talab qiling.')
    cur.close()
...
async def process_customer_callback(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id)
    await bot.send_message(callback_query.from_user.id, 'Siz orqaga qaytingiz',
